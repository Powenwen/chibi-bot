...
export default <BaseCommand>({
    ...
    config: {
        ...
        examples: ["/removerole @user @Member Role violation", "/removerole @user @Muted Unmuting user"],
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "🛡️ Removed by", value: interaction.user.tag, inline: true },
            ...
            // Log the action
            Logger.info(`Role ${role.name} removed from user ${user.tag} by ${interaction.user.tag} in guild ${interaction.guild.name}`);
            ...
            try {
                const dmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`The **${role.name}** role has been removed from you in **${interaction.guild.name}**`)
                    .addFields(
                        { name: "📝 Reason", value: reason, inline: false }
                    )
                    .setTimestamp();
                ...
                Logger.warn(`Failed to send role removal DM to user ${user.id}: ${error}`);
            }

        } catch (error) {
            Logger.error(`Error removing role from user ${user.id}: ${error}`);
            return interaction.reply({
                content: "❌ Failed to remove role. Please check my permissions and try again.",
                ephemeral: true
            });
        }
    }
